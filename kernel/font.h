/*
 * font.h - 8x8 bitmap font for graphics mode
 *
 * in text mode, the VGA card has a built-in font.
 * in graphics mode, there's no font - we have to draw
 * every letter ourselves, pixel by pixel.
 *
 * each character is 8x8 pixels, stored as 8 bytes.
 * each byte is one row, and each bit is one pixel:
 *
...
/*
 * 8x8 bitmap font data - covers ASCII 32 (space) through 126 (~)
 * each character = 8 bytes, one per row
 * bit 7 (leftmost) = leftmost pixel
 *
 * based on the classic IBM/VGA 8x8 font
 */
static const uint8_t font_data[][8] = {
    ...
    /*102: f */      {0x38,0x6C,0x60,0xF8,0x60,0x60,0xF0,0x00},
    ...
    /*121: y */      {0x00,0x00,0xC6,0xC6,0xCE,0x76,0x06,0xC6},
